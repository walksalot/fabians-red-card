...
export default async function globalSetup(config: FullConfig): Promise<void> {
  ...
  // On CI the webServer command seeds BEFORE `next start` boots: the
  // production server's boot hook caches its SQLite handle, and the seed
  // scripts delete the db files, so reseeding here (after boot) would leave
  // the server on the unlinked pre-seed file. Locally `next dev` compiles
  // instrumentation lazily — after this seeding — so the order is safe.
  if (!process.env.CI) {
