...
/**
 * Mid-tournament gameplay e2e suite. Mobile-dark (390x844) against a local
 * server on port 3200, deterministic clock (FAKE_NOW 2026-06-12 — match 1 has
 * kicked off and is locked, match 2 is open), dedicated SQLite db
 * (.data/e2e2.db), live-feed scheduler disabled. Runs after the journey suite
 * (separate config) so the two local servers never run concurrently.
 */
...
export default defineConfig({
  ...
  webServer: {
    // Seed both deterministic databases before boot so the production server's
    // cached SQLite handle sees the fixtures. Local production routing avoids
    // transient `next dev` 404s during combined browser runs; CI behavior is
    // unchanged and consumes the workflow's existing build.
